...
def build_decoder(path_decoder_weights=""):
    """
        ...
        If the checkpoint has a different num_class (e.g. 150 for ADE20k), only matching
        layers are loaded; the final classification layer is left as initialized.
    """
    net_decoder = PPM(num_class=NUM_CLASSES, fc_dim=FC_DIM)
    model_state = net_decoder.state_dict()

    pretrained = path_decoder_weights != ""
    if pretrained:
        print('Loading weights for net_decoder')
        ckpt = torch.load(path_decoder_weights, map_location=lambda storage, loc: storage)
        # Load only keys with matching shapes (e.g. 150-class checkpoint into 2-class model)
        ckpt_filtered = {k: v for k, v in ckpt.items() if k in model_state and v.shape == model_state[k].shape}
        skipped = set(ckpt.keys()) - set(ckpt_filtered.keys())
        if skipped:
            print(f'Skipped {len(skipped)} decoder key(s) (shape mismatch): {sorted(skipped)}')
        net_decoder.load_state_dict(ckpt_filtered, strict=False)
    else:
